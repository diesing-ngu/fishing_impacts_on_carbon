...
Create a mask of fished areas, i.e., all grid cells that have a SVR > 0/yr. This mask is for display purposes and won't be used in the analysis.
...
The proportion of the sediment that resettles after resuspension $Pcrd$ was taken from Sala et al. (2021). By setting $Pcrd = 1$ (all resuspended sediment resettles), $\delta d$ becomes 0 and the mass of vulnerable organic carbon due to changes in organic carbon density ($\delta \rho_G$) can be estimated separately.

```{r thickness_change}
Pcrd <- 0.87
#Pcrd <- 1 #When the effect of changes in organic carbon density should be considered only.
